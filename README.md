...
| 能力 | 目标 | 当前状态 |
|---|---|---|
| Issue 工作流 | 创建、查询和更新 Issue，具备身份隔离与幂等保护 | Schema + create/get Service |
...
| 指标 | 当前结果 | 目标证据 |
|---|---:|---|
| 后端测试 | 37 passed（Day 3 本地） | CI 中的单元、API 与数据库测试 |
...
- 扩展 Issue Service 与 Repository 的 list/update/close 行为；
